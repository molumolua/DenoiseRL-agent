...
DUMP_EXPERIMENT_NAME=${DUMP_EXPERIMENT_NAME:-denoise_grpo_v2_complete_eval}
# Training envs are constructed by the shared entry point even in val-only
# mode. A single rollout slot avoids creating unused GRPO-group env workers.
GROUP_SIZE=${GROUP_SIZE:-1}
source "${SCRIPT_DIR}/params.sh"

if [ -z "${CKPT_DIR:-}" ]; then
  echo "Set CKPT_DIR to a DenoiseRL v2 experiment root or global_step_* directory." >&2
  exit 1
fi
...
echo "============================================================"
echo " DenoiseRL v2 exhaustive ALFWorld evaluation"
echo " split(s)=${EVAL_SPLITS}  val_n=${VAL_N}"
...
python3 -m verl.trainer.main_ppo \
  ...
  "trainer.project_name=${EVAL_PROJECT_NAME:-verl_agent_alfworld_denoise_v2_eval}" \
  "trainer.experiment_name=${EXPERIMENT_NAME:-denoise_grpo_v2_complete_eval}" \
